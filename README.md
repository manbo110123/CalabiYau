...
Tank 网络基线之后的人物主线采用“一问一答、共同决策后实施”的方式推进。请从下列活动文档开始：

1. [总目录与共同决策规则](docs/3C开发陪练手册/00_总目录与共同决策规则.md)
2. [已确认事实与决策总账](docs/3C开发陪练手册/01_已确认事实与决策总账.md)
3. [待访谈问题清单](docs/3C开发陪练手册/02_待访谈问题清单.md)
4. [共同决策动态路线图](docs/3C开发陪练手册/03_共同决策动态路线图.md)

旧助手生成总纲、固定 S00～S38 路线和详细分卷已移入 `docs/3C开发陪练手册/候选方案档案`，没有执行权。当前没有预设的下一个代码阶段；当前入口是愿景访谈 Q001。只有写入决策总账并由开发者明确确认的计划，才能指导 Package、资源、Unity 和代码修改。
