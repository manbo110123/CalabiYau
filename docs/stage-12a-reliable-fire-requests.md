...
本小阶段只补足“客户端的开火意图能否到达服务器”的 UDP 丢包问题；它不是死亡、重生和结算等关键事件账本的完整替代品。`WorldSnapshot` 仍保持独立、可丢弃的全量状态管线；后续 12B.1 明确了 `FireEvent`、`HitEvent`、`HealthChangedEvent` 都不是生命状态权威来源。
...
`accepted=true` 仅表示该请求通过 `GameWorld` 的接收校验并且已经进入本次开火队列；它不表示命中、伤害或扣血。实际开火、延迟补偿、命中检测和生命值修改仍只在下一次权威 `GameWorld.Tick` 中发生；`FireEvent` 与 `HitEvent` 只驱动表现，血量以后续权威快照为准。
...
这里的 `FireRequest` 不是“严格有序可靠开火通道”。客户端只在有限时间内重发同一个 `fireSequence`，服务器在有限历史窗口内对相同序列去重并回放收据；同一序列因此至多入队和结算一次。

不同序列之间不等待缺失序列，也不设置复杂的乱序窗口。例如序列 12 先到、11 因 UDP 延迟后到时，服务器可以把 11 视为过期意图并丢弃，或在尚未被更新序列覆盖时按现有校验处理。这个取舍避免旧包阻塞较新的操作，代价是不能承诺“每一发都按玩家按键顺序严格结算”。若将来需要该承诺，应单独实现有序可靠通道、乱序缓冲和超时策略，而不是把当前有限重发描述成严格有序。
